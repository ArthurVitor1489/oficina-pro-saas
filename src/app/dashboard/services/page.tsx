    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-black tracking-tight text-slate-900 flex items-center gap-2">
            ...
            Catálogo de Serviços ({servicesList.length})
          </h1>
      ...
      {servicesList.length === 0 ? (
        <Card className="text-center py-12">
          ...
          <h3 className="text-base font-bold text-slate-800">Nenhum serviço cadastrado</h3>
          <p className="text-xs sm:text-sm text-slate-500 max-w-sm mx-auto mt-1 mb-4">
            Cadastre os serviços oferecidos pela sua oficina para acelerar orçamentos e ordens de serviço.
          </p>
        </Card>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {servicesList.map((svc) => (
            <Card key={svc.id} className="hover:border-sky-300 transition-colors flex flex-col justify-between">
              <div>
